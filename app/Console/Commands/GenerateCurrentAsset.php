...
class GenerateCurrentAsset extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($platIds as $platId) {
            self::$page = 1;
            while(true) {
                $assetUrl = 'https://www.zhenrongbao.com/plat/plat_credit_assemble?pid=1&current_page='.self::$page.'&credit_plat_id='.$platId['id'].'&_access_token=&platform=pc&_=1470198705937';
                ...
                Log::info('活期资产相关信息插入成功,id为'. $asset->id .',页码为:' . self::$page .'时间为:' . date('Y-m-d H:i:s', time()));
                self::$page++;
            }
        }
