...
$content = file_get_contents('https://www.zhenrongbao.com/plat/plat_assemble?pid=1&current_page=1&_access_token=&platform=pc');
$content = json_decode($content, true);
$pagePlatCount = 0;
if (!empty($content) && is_array($content)) {
    $pagePlatCount = $content['data']['page_count'];
...
for($i=1; $i<=$pagePlatCount; $i++) {
    $pagePlatContent = file_get_contents('https://www.zhenrongbao.com/plat/plat_assemble?pid=1&current_page='. $i .'&_access_token=&platform=pc');
    $pagePlatContent = json_decode($pagePlatContent, true);
    $pagePlatContent  = $pagePlatContent['data']['credit_plat'];
    ...
    foreach ($pagePlatContent as $platInfo) {
        ...
        $pageAssetData = file_get_contents('https://www.zhenrongbao.com/plat/plat_credit_assemble?pid=1&current_page=1&credit_plat_id='. $platId .'&_access_token=&platform=pc');
        $pageAssetData = json_decode($pageAssetData, true);
        $pageAssetCount = $pageAssetData['data']['page_count'];
        for ($j=1; $j<=$pageAssetCount; $j++) {
            $pageAssetContent = file_get_contents('https://www.zhenrongbao.com/plat/plat_credit_assemble?pid=1&current_page=1&credit_plat_id='. $platId .'&_access_token=&platform=pc');  //某个平台下某一页资产的相关信息
            $pageAssetContent = json_decode($pageAssetContent, true);
            $pageAssetContent = $pageAssetContent['data']['credit_assemble'];
            foreach ($pageAssetContent as $assetInfo) {
                file_put_contents('/tmp/asset.log', $assetInfo['credit_name'] . "\n");
